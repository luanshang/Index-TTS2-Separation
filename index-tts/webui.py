...
for file in [
    ...
    if not os.path.exists(file_path):
        print(f"必需文件 {file_path} 不存在。请先下载。")
        sys.exit(1)
...
i18n = I18nAuto(language="zh_CN")  # 强制使用中文
MODE = 'local'

# 尝试加载模型，如果内存不足则使用 CPU
tts = None
try:
    print(">> 正在加载模型到 GPU...")
...
def gen_single(emo_control_method,prompt, text,
    ...
    print(f"情感控制模式:{emo_control_method},权重:{emo_weight},向量:{vec}")
...
with gr.Blocks(title="IndexTTS 演示") as demo:
    mutex = threading.Lock()
    gr.HTML('''
    <h2><center>IndexTTS2: 情感表达与时长可控的自回归零样本文本转语音系统</h2>
<p align="center">
<a href='https://arxiv.org/abs/2506.21619'><img src='https://img.shields.io/badge/ArXiv-2506.21619-red'></a>
    ...
    with gr.Tab(i18n("音频生成")):
        ...
        with gr.Accordion(i18n("高级生成参数设置"), open=False, visible=True) as advanced_settings_group:
            with gr.Row():
                with gr.Column(scale=1):
                    gr.Markdown(f"**{i18n('GPT2 采样设置')}** _{i18n('参数会影响音频多样性和生成速度详见')} [生成策略](https://huggingface.co/docs/transformers/main/en/generation_strategies)._")
                    with gr.Row():
                        do_sample = gr.Checkbox(label="是否采样 (do_sample)", value=True, info=i18n("是否进行采样"))
                        temperature = gr.Slider(label="温度 (temperature)", minimum=0.1, maximum=2.0, value=0.8, step=0.1, info="控制生成的随机性，值越大越随机")
                    with gr.Row():
                        top_p = gr.Slider(label="核采样 (top_p)", minimum=0.0, maximum=1.0, value=0.8, step=0.01, info="保留累计概率达到该值的token")
                        top_k = gr.Slider(label="Top-K (top_k)", minimum=0, maximum=100, value=30, step=1, info="仅考虑概率最高的k个token")
                        num_beams = gr.Slider(label="束搜索数 (num_beams)", value=3, minimum=1, maximum=10, step=1, info="束搜索的束数量，值越大质量越好但速度越慢")
                    with gr.Row():
                        repetition_penalty = gr.Number(label="重复惩罚 (repetition_penalty)", precision=None, value=10.0, minimum=0.1, maximum=20.0, step=0.1, info="防止重复生成")
                        length_penalty = gr.Number(label="长度惩罚 (length_penalty)", precision=None, value=0.0, minimum=-2.0, maximum=2.0, step=0.1, info="控制生成长度偏好")
                    max_mel_tokens = gr.Slider(label="最大Mel Token数 (max_mel_tokens)", value=1500, minimum=50, maximum=tts.cfg.gpt.max_mel_tokens, step=10, info=i18n("生成Token最大数量，过小导致音频被截断"), key="max_mel_tokens")
                    # with gr.Row():
        ...
        example_table = gr.Dataset(label="示例",
            samples_per_page=20,
    ...
    def on_example_click(example):
        print(f"示例已点击: ({len(example)} 个值) = {example!r}")
        return (
